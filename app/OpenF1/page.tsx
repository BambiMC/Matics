...
const Homepage: React.FC = () => {
    ...
    const [queryText, setQueryText] = useState<string[]>(['driver_number=55&session_key=9159&speed>=300', 'driver_number=1&session_key=9158', 'session_key=9165&interval<0.005', 'session_key=9161&driver_number=63&lap_number=8', 'session_key=9161&driver_number=81&date>2023-09-16T13:03:35.200&date<2023-09-16T13:03:35.800', 'year=2023&country_name=Singapore', 'session_key=9158&pit_duration<31', 'meeting_key=1217&driver_number=40&position<=3', 'flag=BLACK AND WHITE&driver_number=1&date>=2023-01-01&date<2023-09-01', 'country_name=Belgium&session_name=Sprint&year=2023', 'session_key=9165&tyre_age_at_start>=3', 'session_key=9158&driver_number=11', 'meeting_key=1208&wind_direction>=130&track_temperature>=52']);
    ...
    const [applicableAttributes, setApplicableAttributes] = useState<string[]>([]);


    const fetchData = async () => {
        try {
            console.log(`https://api.openf1.org/v1/${selectedCategory[1]}?${queryText}`);
            const response = await fetch(`https://api.openf1.org/v1/${selectedCategory[1]}?${queryText}`);
            if (!response.ok) {
    ...
    const handleCategoryChange = (label: string[]) => {
        ...
        const index: number = apiItems.indexOf(label);
